...
class TokenPrinter:
    ...
    def __init__(self, source, names, output_file):
        self.indent = 0
        self.first_on_line = 1
        self.symboltimes = {}
        self.names = names
    ...
    def line_append(self, s):
        ...
        if self.pending:
            ...
            if conf.blanks == conf.OBFUSCATE_BLANKS:
                ...
                self.output_file.write(b''.join(self.pending))
            self.pending = []
        
        if self.first_on_line:
            # sys.stdout.buffer.write(b" "*self.indent)
            self.output_file.write(b" "*self.indent)
        else:
            # sys.stdout.buffer.write(b" "*TOKENBLANKS)
            self.output_file.write(b" "*TOKENBLANKS)

        # sys.stdout.buffer.write(s.encode(self.encoding))
        self.output_file.write(s.encode(self.encoding))
...
class Configuration:
    ...
    def __init__(self):
        ...
        self.file = args[0]
        if os.path.isfile(self.file) and self.file.split('.')[-1] == 'py':
            self.output_file = open("{}_obfuscate.py".format(self.file.split('.')[0]), "wb+")
        else:
...
def main():
    ...
    TokenPrinter(source, ce.result, conf.output_file)
    
    # Step 5: Output a marker that makes it possible to recognize
    # obfuscated files
    conf.output_file.write(b"# 271e772380b6309ceae3df4d6b5515c47a49063f827aab6a076c04e6d69bed5f")
